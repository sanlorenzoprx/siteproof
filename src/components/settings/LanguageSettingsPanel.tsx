...
function LanguageSelect({ label, value, onChange }: { label: string; value: SiteProofLanguage; onChange: (value: SiteProofLanguage) => void }) {
  return (
  ...
  return (
    <section className="bg-white rounded-3xl border border-slate-200 p-6 space-y-5">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <LanguageSelect label="UI Language" value={settings.uiLanguage} onChange={(uiLanguage) => void updateSettings({ uiLanguage })} />
        <LanguageSelect label={t('capture.captureLanguage')} value={settings.captureLanguage} onChange={(captureLanguage) => void updateSettings({ captureLanguage })} />
        <LanguageSelect label={t('reports.reportLanguage')} value={settings.exportLanguage} onChange={(exportLanguage) => void updateSettings({ exportLanguage })} />
      </div>
    </section>
  );
}
