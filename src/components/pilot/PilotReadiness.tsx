    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      <header className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Pilot Readiness</h1>
            <p className="text-sm text-slate-500 font-medium">Field pilot checks for offline use, sync, templates, media, and exports.</p>
          </div>
        </div>
        <button
        ...
        >
          <RotateCw size={14} className={cn(loading && 'animate-spin')} /> Refresh
        </button>
      </header>

      <section className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
        {loading || !report ? (
          <div className="py-12 text-center text-slate-500 font-bold">Checking pilot readiness…</div>
        ) : (
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 text-slate-900 font-black text-lg">
                    <ShieldCheck size={20} />
                    {report.status === 'pilot_ready' ? 'Ready for small pilot' : report.status === 'blocked' ? 'Blocked before pilot' : 'Needs attention'}
                  </div>
                  <p className="text-sm text-slate-500 font-medium">Generated {new Date(report.generatedAt).toLocaleString()}</p>
                </div>
              </div>
              <div className="text-xs font-bold text-slate-500 max-w-md">
                Use this before handing the app to a contractor. Warnings can be acceptable for offline-only demos; failed checks should be fixed first.
              </div>
            </div>
                ...
                <div key={check.id} className="rounded-2xl border border-slate-200 p-4 bg-slate-50/60">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {check.action && <p className="text-[11px] text-blue-700 font-bold mt-2">Next: {check.action}</p>}
                    </div>
                  </div>
                </div>
              ))}
